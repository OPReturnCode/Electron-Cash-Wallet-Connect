...
class WCClient:
    ...
    @classmethod
    def from_wc_uri(cls, wc_uri_str, socks_opts):
        ...
        if wc_uri_str.find("@1?") >= 0:
            # v1
            return WCv1Client.from_wc1_data(found[0], socks_opts)
        if wc_uri_str.find("@2?") >= 0:
            # Is it a V2 ?
            if wc_uri_str.find("publicKey=") >= 0:
                # v2 early waku
                return WCv2ClientLegacy.from_wc2_uri(wc_uri_str, socks_opts)
            if wc_uri_str.find("symKey=") >= 0:
                # latest v2 irn
                return WCv2Client.from_wc2_uri(wc_uri_str, socks_opts)
    ...
    def reconnect(self):
        ...
        try:
            self.websock = WebSocketClient(self.relay_url, self.origin_domain, socks_opts=self.socks_opts)
